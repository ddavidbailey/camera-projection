    ...
    <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
      <header className="flex flex-col gap-3.5 mb-1.5">
        ...
        <h1 className="font-heading font-normal text-[clamp(30px,3.6vw,40px)] leading-[1.08] tracking-[-0.015em] m-0 text-(--color-foreground) text-balance">
          Set the <em className="italic text-(--color-primary) pr-[0.06em]">paper</em>. We&apos;ll find it.
        </h1>
        <p className="text-[14.5px] leading-[1.55] text-(--color-muted) m-0 max-w-[38ch] text-pretty">
          A workshop account lets you upload worksheets, connect cloud storage, and share expiring camera links with your room.
        </p>
      </header>
      ...
      <div className="grid grid-cols-2 gap-3 max-sm:grid-cols-1">
        ...
        <Field label="Workshop" hint="optional" value={org} onChange={setOrg} placeholder="Hatch Studio" autoComplete="organization" />
      </div>

      <Field label="Email" type="email" value={email} onChange={setEmail} placeholder="you@studio.io" autoComplete="email" />
      <PasswordField label="Password" value={pwd} onChange={setPwd} showStrength autoComplete="new-password" />

      <CheckBox checked={agree} onChange={setAgree} block>
        <span>
          I agree to the{" "}
          <a href="#" className="text-inherit underline decoration-1 underline-offset-2">terms</a>
          {" "}and{" "}
          <a href="#" className="text-inherit underline decoration-1 underline-offset-2">privacy policy</a>.
        </span>
      </CheckBox>
      ...
      <button type="submit" disabled={!agree}
        ...
        <span className="relative">{submitting ? "Setting up…" : "Create account"}</span>

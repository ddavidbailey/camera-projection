...
import { useMode, usePalette } from "./AuthProvider";
    ...
    <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
      <header className="flex flex-col gap-3.5 mb-1.5">
        ...
        <h1 className="font-heading font-normal text-[clamp(30px,3.6vw,40px)] leading-[1.08] tracking-[-0.015em] m-0 text-(--color-foreground) text-balance">
          Pick up where <em className="italic text-(--color-primary) pr-[0.06em]">the light</em> left off.
        </h1>
        <p className="text-[14.5px] leading-[1.55] text-(--color-muted) m-0 max-w-[38ch] text-pretty">
          Sign back into your workshop. Your worksheets and active session links are right where you parked them.
        </p>
      </header>
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 my-0.5">
        ...
        <span className="font-code text-[10.5px] tracking-[0.16em] uppercase text-(--color-muted) whitespace-nowrap">or with email</span>
        <span className="h-px bg-[linear-gradient(90deg,transparent,var(--color-border),transparent)]" />
      </div>

      <Field label="Email" type="email" value={email} onChange={setEmail} placeholder="you@studio.io" autoComplete="email" />
      <PasswordField label="Password" value={pwd} onChange={setPwd} autoComplete="current-password" />

      <div className="flex items-center justify-between gap-3 flex-wrap">
        <CheckBox checked={remember} onChange={setRemember}>Keep me signed in</CheckBox>
        <a href="#" className="text-[13px] text-(--color-muted) no-underline border-b border-dashed border-(--color-border) pb-px transition-[color,border-color] duration-180 hover:text-(--color-foreground) hover:border-(--color-foreground) whitespace-nowrap">
          Forgot password?
        </a>
      </div>
      ...
      <button type="submit"
        ...
        <span aria-hidden="true" className="absolute inset-0 -translate-x-full bg-[linear-gradient(90deg,transparent,color-mix(in_oklab,var(--color-secondary),transparent_50%),transparent)] group-hover:translate-x-full transition-transform duration-550" />
        <span className="relative">{submitted ? "Aligning camera…" : submitting ? "Tracing…" : "Sign in"}</span>
        <span className={`relative inline-flex ${submitting ? "animate-auth-trace" : ""}`}>
      ...
      <p className="text-center text-[13px] text-(--color-muted) mt-1.5 text-balance">
        New here?{" "}
        <button type="button" onClick={() => setMode("up")}
          className="bg-transparent border-none p-0 font-[inherit] text-(--color-foreground) cursor-pointer border-b border-b-(--color-foreground) pb-px hover:text-(--color-primary) hover:border-b-(--color-primary)">
          Create an account
        </button>
      </p>
    </form>

...
export function AuthAside() {
  ...
  return (
    <aside
      ...
      style={{
        background:
          "linear-gradient(180deg, color-mix(in oklab, var(--color-background), white 2%) 0%, var(--color-background) 100%)",
      }}
    >
      {/* Brand row */}
        ...
        >
          ...
          <div className="absolute top-3 left-3.5 right-3.5 flex items-center gap-2 font-code text-[8px] tracking-[0.12em] uppercase text-(--color-muted) z-[5]">
            <span
              ...
              style={{
                background:
                  "color-mix(in oklab, var(--color-surface), transparent 30%)",
              }}
            >
          ...
          >
            ...
            {/* Projection cone */}
            <path
              d="M 143 60 L 40 268 L 260 268 L 157 60 Z"
              fill="url(#authConeGrad)"
            />
            <line
              x1="143"
              y1="60"
            ...
            <path
              d="M 90 105 L 210 105 L 260 268 L 40 268 Z"
            ...
            <g clipPath="url(#authPaperShape)">
              <g clipPath="url(#authPaperReveal)">
                ...
                <line
                  ...
                  x2="188"
                  y2="148"
                  stroke="color-mix(in oklab, var(--color-foreground), transparent 65%)"
                  strokeWidth="1.4"
                  strokeLinecap="round"
                />
                <line
                ...
                <line
                  ...
                  x2="86"
                  y2="258"
                  stroke="color-mix(in oklab, var(--color-foreground), transparent 80%)"
                  strokeWidth="1.2"
                  strokeLinecap="round"
                />
                <line
            ...
            <line
              ...
              style={{
                filter:
                  "drop-shadow(0 0 4px color-mix(in oklab, var(--color-secondary), transparent 30%))",
              }}
            >
              <animate
                attributeName="y1"
                values="105;268;268;105;105"
                keyTimes="0;0.42;0.5;0.92;1"
                dur="6s"
                repeatCount="indefinite"
                calcMode="spline"
          ...
          <div className="absolute left-3.5 right-3.5 bottom-3 flex items-center gap-2 font-code text-[9.5px] tracking-[0.12em] uppercase text-(--color-muted) z-[5]">
            ...
            <span
              style={{
                color:
                  "color-mix(in oklab, var(--color-muted), transparent 30%)",
              }}
            >
              anchored · 4 corners
            </span>
      ...
      <div className="border-t border-(--color-border) pt-6">
        <p className="font-heading text-[22px] leading-[1.3] text-(--color-foreground) m-0 mb-3 tracking-[-0.005em] text-pretty">
          ...
          <em className="italic text-(--color-primary)">on the paper</em> — no
          printing, no projector hung from a ceiling.&rdquo;
        </p>
        <p className="font-code text-[10.5px] uppercase tracking-widest text-(--color-muted) m-0">
      ...
      <div className="flex justify-between gap-3 font-code text-[10.5px] uppercase tracking-[0.08em] text-(--color-muted)">
        <span>{mode === "in" ? "01 / sign in" : "02 / Sign up"}</span>
